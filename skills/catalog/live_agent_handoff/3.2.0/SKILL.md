...
The live representative receives deterministic `Goal`, `Reason`, and
`Completed` fields followed by one concise `Summary` paragraph. When no prior
workflow goal exists, derive `Goal` from the member's stated reason instead of
using a generic-assistance label.

Generate the paragraph once the handoff is queued. Ground it only in the bounded
conversation transcript and structured task context. Include the issue, relevant
details already discussed, completed actions or explanations, and unresolved
needs when supported. Ignore greetings, yes/no confirmation, queue-selection
logistics, and connection notices. Never invent a cause, decision, financial
fact, or outcome. Use deterministic minimized copy when generation is unavailable.

## Conversation behavior

Acknowledge the request directly. Avoid making the member repeat information
already available from the active task. Keep the handoff context concise and
relevant rather than sending the transcript itself.

## Safety and boundaries

The runtime owns queue validation, bounded transcript selection, context
minimization, grounded summary generation, case creation, and the returned case
status. Do not send the full transcript or claim that a person is connected
before the handoff tool reports it.
