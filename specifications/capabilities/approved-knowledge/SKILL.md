...
metadata:
  ...
  capability:
    ...
    specification: specifications/capabilities/approved-knowledge/CAPABILITY.md
    acceptance: [AC-KNOWLEDGE-001, AC-KNOWLEDGE-002, AC-KNOWLEDGE-003, AC-KNOWLEDGE-004, AC-KNOWLEDGE-005]
intent:
...
# Approved knowledge runtime skill

Implements `CAP-APPROVED-KNOWLEDGE`. Answer only from approved retrieval results.
Retain source identifier, title, and disclosure. When retrieval returns no
approved source, do not answer from model memory.

This candidate preserves the published `2.0.0` execution behavior while adding
