...
metadata:
  ...
  capability:
    ...
    specification: specifications/capabilities/guided-balance/CAPABILITY.md
    acceptance: [AC-BALANCE-001, AC-BALANCE-002, AC-BALANCE-003, AC-BALANCE-004, AC-BALANCE-005, AC-BALANCE-006]
intent:
...
# Guided balance runtime skill

Implements `CAP-GUIDED-BALANCE`. Retrieve only accounts eligible for the
authenticated member, minimize clarification, mask identifiers, and restart
dependent selection when the account type changes.

This candidate preserves the published `2.1.0` execution behavior while adding
